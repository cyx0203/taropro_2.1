import { StepNext, UpateCurrentPageData, Warn,
    DataCache, StepContent, ajaxparams
} from "../../core/engine";
import JsUtil from "../utils/jsutil";

/**
 * 单个业务配置
 * @param {*} func 
 * @returns 
 */
const setAjaxConfig = (func)=>{
    var config = {
        type: 'ajax',
        config: {
            success: function (re) {
                const res = re;
                let returnCode = res.rspCode;
                if(returnCode==='00'){
                    return func.success(res);
                }
                if(func.error) func.error({code: returnCode, msg: res.rspMsg});
                else Warn(res.ResultMsg);
            },
            complete:()=>{
                console.log("ajax complete invoke")
            }
        },
        ready:() => {
            if(func.ready) func.ready();
            ajaxparams.REQ_BODY.hospitalId = JsUtil.param.hospitalId;
        },
        load: () => {
            ajaxparams.REQ_BODY = {};
            if(func.load) func.load();
        }

    };
    return config;
}

export const Ajax_Test = class {
    static data(){
        return setAjaxConfig({
            success:(data)=>{
                console.log("suc data = ", data.listInfo);
                const list = data.listInfo;
                const patInfo = list[0];
                if(JsUtil.isEmpty(list) || patInfo.useFlag!=="Y"){
                    Warn("未找到启用的患者信息");
                    return;
                }
                let SPatInfo = StepContent.PatInfo;
                SPatInfo.Name = patInfo.patName;
                SPatInfo.IdNo = patInfo.patIdno;
                SPatInfo.Sex = patInfo.patSex;
                SPatInfo.Age = patInfo.patAge;
                SPatInfo.Phone = patInfo.phoneNo;
                SPatInfo.HisCardNo = patInfo.cardNo;
                SPatInfo.PatientId = patInfo.patId;
                SPatInfo.PatientMzh = JsUtil.sEmpty(patInfo.patNo);
                StepContent.PatInfo = SPatInfo;
                JsUtil.logger.log("PatInfo = ", StepContent.PatInfo)
                UpateCurrentPageData({msg: "当前就诊人:"+SPatInfo.Name})
            },
            error:(err)=>{
                Warn(err.msg);
            },
            ready:()=>{
                ajaxparams.REQ_CODE = "0002_Rhms";
                ajaxparams.REQ_BODY = {
                    tradeCode: "appuserpatient.select",
                    hospitalId: JsUtil.param.hospitalId,
                    openId: StepContent.OpenId,
                    activeFlag: "Y"
                }
            }
        })
    }
}


export const Ajax_Test2 = class {
    static data(){
        return {
            type: "ajax",
            config:{
                success:(res)=>{
                    console.log("121212, ", res)
                    let returnCode = res.returnCode;
                    if(returnCode==='0000'){
                        return func.success(res);
                    }
                    if(func.error) func.error({code: returnCode, msg: res.returnInfo});
                    else Warn(res.returnInfo);
                },
                error:(errorInfo)=>{

                }
            },
            // load:()=>{ },
            ready:()=>{
                ajaxparams.REQ_CODE = "0001_His";
                ajaxparams.REQ_BODY = {
                    FuncName: "GetMZPatientInfo",
                    tradeType: "H000",
                    tradeName: "测试",
                }
            }
        }
    }
}


export const V1_Ajax_Test = class {
    static data(){
        return setAjaxConfig({
            success:(data)=>{
                console.log("suc data = ", data);
                // 直接更新当前页面的数据
                UpateCurrentPageData({msg: "从ajx交易中获取到用户姓名="+data.PatInfo.PatientName+","+(Math.random()*1000).toFixed(0)},)
                // StepNext(1);
            },
            error:(err)=>{
                Warn(err.msg);
            },
            ready:()=>{
                ajaxparams.REQ_CODE = "0001_His";
                ajaxparams.REQ_BODY = {
                    FuncName: "GetMZPatientInfo",
                    tradeType: "H000",
                    tradeName: "测试",
                }
            }
        })
    }
}