...
import {AtTag, AtAvatar, AtList, AtListItem, AtFloatLayout, AtButton, AtSearchBar, AtTabs, AtTabsPane, AtGrid, AtInput} from "taro-ui";
...
export default function main() {
  ...
  const GPage = usePage({
    ...
    pageName: "h5_pages_IH_index_index4_2_index",
    //用于接收外部发送的更新数据的事件
    ndaExcute: (nda: any) => {
      console.log('NDA');
      console.log(nda);
  ...
  return (
    <Root hashData={styles}>
      ...
      <View className={styles.index4_main} style={{ height: viewHeight }}>
        ...
        {/* 用户顶部BAR,搜索那一行 */}
        <View className={styles.index4_userbar+' '+styles.index4_topbg_pure_color}>
          {/* 用户信息区域 */}
                  ...
                  return (
                    <View onClick={() => {
                      ...
                      <View style={{ paddingLeft: '15%' }}>
                        <View className={styles.index4_banner_main_text} >{item.title}</View>
                        <View className={styles.index4_banner_sub_text}>{item.subTitle}</View>
                      </View>
                    </View>
