...
export default function main() {
  ...
  const GPage = usePage({
    ...
    pageName: "h5_pages_TJXRMYY_paymentList_index",
    //用于接收外部发送的更新数据的事件
    ndaExcute: (nda: any) => {
  ...
  const [tabCurrent, setTabCurrent] = useState(GPage.Data.currentTabIndex || 0);
  ...
  useEffect(() => {
    unpaidsArea &&
      !_.isEmpty(unpaidsArea.unpaidList) &&
      calTotal(unpaidsArea.unpaidList);
    if (GPage.Data.isDisabled) {
      //默认全选
      // allSelect();
      calTotal(unpaidsArea.unpaidList);
      setRadioBtn(true);
  ...
  return (
    <Root hashData={styles}>
      <View className={styles.paymentList}>
        ...
        {payBtnText && footerArea && (
          <View
            className={styles.footer}
            style={{
              display: tabCurrent === 1 ? "none" : ""
            }}
          >
            <View className={styles.paySum}>
              {!GPage.Data.isRadio && footerArea && tabCurrent === 0 ? (
                <Radio
                  checked={RadioBtn}
                  onClick={() => {
                    if (GPage.Data.isDisabled) return;
                    let flag = RadioBtn;
                    flag = !flag;
                    setRadioBtn(flag);
                    allSelect();
            ...
            <View
              ...
              onClick={() => {
                mjcom.GotoMockPage(`h5/mock/TJXRMYY/NATType_01.js`, "main");
