...
export default function main() {
  ...
  const GPage = usePage({
    ...
    pageName: "h5_pages_TJXRMYY_paymentList02_index",
    //用于接收外部发送的更新数据的事件
    ndaExcute: (nda: any) => {
  ...
  const handleCheckedData = value => {
    ...
    if (GPage.Data.isRadio && GPage.Data.isRadio) {
      ...
      for (let i = 0; i < dataObj.length; i++) {
        if (dataObj[i].id === value.id)
          dataObj[i] = value;
        if (dataObj[i].checked) flag++;
        if (flag === dataObj.length) {
          setRadioBtn(true);
          GPage.DoWXCall("allInClick", dataList, !RadioBtn);
  ...
  return (
    <Root hashData={styles}>
      <View className={styles.paymentList02}>
        <View
        ...
        >
          {GPage.Map(
            ...
            (i, j) => {
              return (
                <AtTabsPane current={tabCurrent} index={j}>
