...
const CPSNAME = 'comp_userEcard_02';
const Comp_UserECard_02 = (props: any) => {
    //数据
    ...
    return <Root className={props.className} cpsName={CPSNAME}>
        {
            ...
            <View className='cardTop'
              style={{
                    backgroundImage: t.cardBg?`url(${getIconRes('active',t.cardBg)})`:'',
            ...
            >
                ...
                {
                    t.content?t.content.map((item,index)=>{
                        return(
                            <View className={getClass(index)} key={index}>{item.key?item.key:''}{item.value?item.value:''}</View>
                        )
                    }):''
                }</View>
                {
            ...
            <View className='cardBottom' onClick={()=>rClick(i)}><Radio className='cardRadio' style={{display:'flex',alignItems:'center'}} checked={cardCheck[i]}></Radio>{t.cardTxt?t.cardTxt:''}</View>
            {
               t.extend?
