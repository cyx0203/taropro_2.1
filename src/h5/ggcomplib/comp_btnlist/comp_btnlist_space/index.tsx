...
const CPSNAME = 'comp_btnlist_space';
const Comp_BtnList_Space = (props: any) => {
    //数据
            ...
            data&&data.listData ?
                data.listData.map((item,index)=>{
                    ...
                    <View className='btnlist at-row' onClick={()=>{
                        ...
                        <View className='at-col at-col-2'>{data.hasOwnProperty('arrow')&&data.arrow===true?<AtIcon value='chevron-right' size='30' color='darkgray'></AtIcon>:''}</View>
                    </View>
                    </View> 
                    )
                })
